...
struct ChartView: View {
    ...
    @ObservedObject var viewController: ChartViewController
    
    @State var indicatorViewIsVisible: Bool = false
    @State var changedColorForRegularMarketPrice: UIColor = .clear
    
    var quote: StatisticsQuote? {
        viewController.statistics?.optionChain?.result?.first?.quote ?? nil
    }
    ...
    var body: some View {
            ...
            VStack {
                HStack(alignment: .center, spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        Spacer()
                        Text(verbatim: self.viewController.symbol ?? "").font(.headline).fontWeight(.bold)
                        Spacer()
                        Text(verbatim: self.viewController.statistics?.optionChain?.result?.first?.quote?.shortName ?? "").font(.subheadline).fontWeight(.light).foregroundColor(.secondary)
                        Spacer()
                    }
                    .padding(.leading, 4)
                    .frame(width: geometry.size.width * 0.35, alignment: Alignment(horizontal: .leading, vertical: .center))
                    
                    GraphViewUIKit(viewController: self.viewController).frame(width: geometry.size.width * 0.40, alignment: Alignment(horizontal: .center, vertical: .center))
        ...
        }.onReceive(self.viewController.$statistics, perform: { statistics in
            if let regularPrice = self.quote?.regularMarketPrice, let regularPriceNewValue = self.viewController.regularPriceNewValue {
...
struct ChartView_Previews: PreviewProvider {
    static var previews: some View {
        ChartView(viewController: ChartViewController(withJSON: "AAPL")).frame(height: 60)
    }
}
