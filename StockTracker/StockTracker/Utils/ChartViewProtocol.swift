...
protocol ChartViewProtocol: ObservableObject, Identifiable {
    ...
    var chart: HistoryChart? { get set }
    var statistics: Statistics? { get set }
    ...
    func start()
    func getChartExtremumsFrom(_ historyChart: HistoryChart?)
    func getTimeMarkerCount(from historyChart: HistoryChart?)
...
extension ChartViewProtocol {
    ...
    func getChartExtremumsFrom(_ historyChart: HistoryChart?) {
        if let quote = historyChart?.chart?.result?.first??.indicators?.quote?.first {
    ...
    func getTimeMarkerCount(from historyChart: HistoryChart?) {
        guard let meta = historyChart?.chart?.result?.first??.meta, let timeStamp = historyChart?.chart?.result?.first??.timestamp, let timeStampCount = historyChart?.chart?.result?.first??.timestamp?.count else { return }
