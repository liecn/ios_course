...
struct IndicatorView : View {
    ...
    var timeStamp: [Int] { self.viewController.chart?.chart?.result?.first??.timestamp ?? [] }
    var meta: Meta? { self.viewController.chart?.chart?.result?.first??.meta }
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ...
            ZStack {
                if self.timeStamp.count > 0 {
                    ...
                    Path { path in
                        ...
                        let index = Int(self.positionIndicator * CGFloat(self.viewController.timeMarkerCount ?? self.timeStamp.count))

                        if index >= 0 {
                            DispatchQueue.main.async { self.timeStampIndex = index }
                            if index < self.timeStamp.count {
                                let circlePositionY = self.coordinateY(price: self.viewController.priceForIndex(index), size: geometry.size)
            ...
            .onChanged({ value in
                ...
                let divide = (CGFloat(self.timeStamp.count - 1) / CGFloat(self.viewController.timeMarkerCount ?? self.timeStamp.count))
    ...
    private func coordinateY(price: Double, size: CGSize) -> CGFloat {
        ...
        guard let meta = meta, let chartPreviousClose = meta.chartPreviousClose, let extremums = viewController.chartExtremums else { return 0 }
...
struct IndicatorView_Previews : PreviewProvider {
    static var previews: some View {
        ZStack {
            IndicatorView(indicatorViewIsVisible: .constant(false), timeStampIndex: .constant(0), viewController: DetailChartViewController(withJSON: "AAPL"))
        }
            .colorScheme(.dark)
    }
}
