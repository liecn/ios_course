...
class GraphView<ViewController>: UIView where ViewController: ChartViewProtocol {
    
    @ObservedObject var viewController: ViewController
    
    var lineWidth: CGFloat = 2
    
    init(chartViewController: ObservedObject<ViewController>) {
...
struct GraphViewUIKit<ViewController>: UIViewRepresentable where ViewController: ChartViewProtocol {
    @ObservedObject var viewController: ViewController
 
    func makeUIView(context: UIViewRepresentableContext<GraphViewUIKit<ViewController>>) -> UIView {
        let graphView = GraphView(chartViewController: _viewController)
        graphView.backgroundColor = .clear
        return graphView
    }
    
    func updateUIView(_ uiView: UIView, context: UIViewRepresentableContext<GraphViewUIKit<ViewController>>) {
...
struct GraphViewUIKit_Previews: PreviewProvider {
    static var previews: some View {
        GraphViewUIKit(viewController: DetailChartViewController(withJSON: "AAPL"))
    }
}
